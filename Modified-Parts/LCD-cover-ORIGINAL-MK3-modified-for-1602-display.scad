// Modified for 16x02 LCD instead of 20x04 by Lubomir Rintel
// From: https://raw.githubusercontent.com/prusa3d/Original-Prusa-i3/MK3S/Printed-Parts/SCAD/LCD-cover-ORIGINAL-MK3.scad
...
module main_body()
{
    difference() 
    {
        union() 
        {
            ...
            translate( [ -53.5-1.5 , ss-1.5 , 0 ] ) cube( [ awx+3 , awy+3 , 2+1 ] );          
        }
...
*/
        ...
        translate( [ -53.5 , ss , -1 ] ) cube( [ awx , awy , 10 ] );          
...
/*
    // ORIGINAL PRUSA text
    translate([-60,31,0.6]) rotate([180,0,10]) linear_extrude(height = 2) 
    { text("TOTALLY",font = "helvetica:style=Bold", size=5, center=true); }
    translate([-67,51,0.6]) rotate([180,0,0]) linear_extrude(height = 2) 
    { text("ORIGINAL",font = "helvetica:style=Bold", size=7, center=true); }    
    translate([-18,46,0.6]) rotate([180,0,-6]) linear_extrude(height = 2) 
    { text("PRUSA",font = "helvetica:style=Bold", size=11, center=true); }

    translate([25,47,0.6]) rotate([180,0,5]) linear_extrude(height = 2) 
    { text("HONEST",font = "helvetica:style=Bold", size=4, center=true); }    
...
/*
        ...
        ss=13; // 8
        translate( [ -61.5 , 1 , 1.2 ] ) cube( [ 98.5 , 42 , 10 ] ); 
        translate( [ -52.5 , ss , -1 ] ) cube( [ awx , awy , 10 ] );          
        
*/

    // LCD corners
    translate( [ -53.5 , ss+1.5 , -5.2 ] ) rotate([45,0,0]) cube( [ awx , 5 , 5 ] );  // LCD window
    translate( [ -53.5 , ss+awy /*ss+awy+37 */, -5.2 ] ) rotate([45,0,0]) cube( [ awx , 5 , 5 ] );  // LCD window
